#!/usr/bin/env python3
"""
Dell OME Template v2.0 - Fixed Version
Fixes critical issues found in analysis
"""
...
def create_fixed_template():
    ...
    ET.SubElement(zabbix_export, 'date').text = '2026-03-31T17:00:00Z'
    ...
    ET.SubElement(template, 'description').text = 'Dell OME API monitoring template v2.0 FIXED. Auto-discovery, Status/Power/Temperature/Health, Inventory (CPU/Memory/Disk/RAID/PSU/Fans/Network), Power Metrics, Utilization, GPU, Alerts, Session control.'
    ...
    for macro_name, value, desc in macro_definitions:
        ...
        ET.SubElement(macro, 'value').text = value if value else ''
    ...
    # Item Prototypes - FIXED VERSION
    item_prototypes = ET.SubElement(dr, 'item_prototypes')
    
    prototypes = [
        # Basic (v1.1) - FIXED: units in <units> element, not preprocessing
        ('{#DeviceName}: Status', 'ome.device[{#Id}.status]', '1m', '3', 'Dell OME - Status', 
         '{$OME_HOST}/api/DeviceService/Devices({#Id})', '$.Status', '', ''),
        ('{#DeviceName}: Power State', 'ome.device[{#Id}.power]', '1m', '3', 'Dell OME - Power', 
         '{$OME_HOST}/api/DeviceService/Devices({#Id})', '$.PowerState', '', ''),
        ('{#DeviceName}: Model', 'ome.device[{#Id}.model]', '1m', '4', 'Dell OME - Discovery', 
         '{$OME_HOST}/api/DeviceService/Devices({#Id})', '$.Model', '', ''),
        ('{#DeviceName}: Service Tag', 'ome.device[{#Id}.tag]', '1m', '4', 'Dell OME - Discovery', 
         '{$OME_HOST}/api/DeviceService/Devices({#Id})', '$.DeviceServiceTag', '', ''),
        ('{#DeviceName}: Identifier', 'ome.device[{#Id}.identifier]', '1m', '4', 'Dell OME - Discovery', 
         '{$OME_HOST}/api/DeviceService/Devices({#Id})', '$.Identifier', '', ''),
        ('{#DeviceName}: Connection State', 'ome.device[{#Id}.connection]', '1m', '3', 'Dell OME - Status', 
         '{$OME_HOST}/api/DeviceService/Devices({#Id})', '$.ConnectionState', '', ''),
        ('{#DeviceName}: Temperature', 'ome.device[{#Id}.temperature]', '2m', '3', 'Dell OME - Temperature', 
         '{$OME_HOST}/api/DeviceService/Devices({#Id})/Temperature', '$.Temperature[0].Reading', '', '°C'),
        ('{#DeviceName}: Health Status', 'ome.device[{#Id}.health]', '2m', '3', 'Dell OME - Health', 
         '{$OME_HOST}/api/DeviceService/Devices({#Id})/SubSystemHealth', '$.HealthState', '', ''),
        
        # CPU Inventory
        ('{#DeviceName}: CPU Count', 'ome.device[{#Id}.cpu.count]', '{$INVENTORY_UPDATE_INTERVAL}', '3', 'Dell OME - CPU', 
         '{$OME_HOST}/api/DeviceService/Devices({#Id})/InventoryDetails(\'serverProcessors\')', '$.InventoryInfo..Id', 'length', ''),
        ('{#DeviceName}: CPU Total Cores', 'ome.device[{#Id}.cpu.cores]', '{$INVENTORY_UPDATE_INTERVAL}', '3', 'Dell OME - CPU', 
         '{$OME_HOST}/api/DeviceService/Devices({#Id})/InventoryDetails(\'serverProcessors\')', '$.InventoryInfo..NumberOfCores', 'sum', ''),
        ('{#DeviceName}: CPU Model', 'ome.device[{#Id}.cpu.model]', '{$INVENTORY_UPDATE_INTERVAL}', '4', 'Dell OME - CPU', 
         '{$OME_HOST}/api/DeviceService/Devices({#Id})/InventoryDetails(\'serverProcessors\')', '$.InventoryInfo[0].BrandName', '', ''),
        ('{#DeviceName}: CPU Max Speed', 'ome.device[{#Id}.cpu.maxspeed]', '{$INVENTORY_UPDATE_INTERVAL}', '3', 'Dell OME - CPU', 
         '{$OME_HOST}/api/DeviceService/Devices({#Id})/InventoryDetails(\'serverProcessors\')', '$.InventoryInfo[0].MaxSpeed', '', 'MHz'),
        
        # Memory Inventory
        ('{#DeviceName}: Memory Total Size', 'ome.device[{#Id}.memory.total]', '{$INVENTORY_UPDATE_INTERVAL}', '3', 'Dell OME - Memory', 
         '{$OME_HOST}/api/DeviceService/Devices({#Id})/InventoryDetails(\'serverMemoryDevices\')', '$.InventoryInfo..Size', 'sum', 'MB'),
        ('{#DeviceName}: Memory Modules Count', 'ome.device[{#Id}.memory.count]', '{$INVENTORY_UPDATE_INTERVAL}', '3', 'Dell OME - Memory', 
         '{$OME_HOST}/api/DeviceService/Devices({#Id})/InventoryDetails(\'serverMemoryDevices\')', '$.InventoryInfo..Id', 'length', ''),
        
        # Storage Inventory - FIXED: regex with proper parameters
        ('{#DeviceName}: Physical Disk Count', 'ome.device[{#Id}.disk.count]', '{$INVENTORY_UPDATE_INTERVAL}', '3', 'Dell OME - Storage', 
         '{$OME_HOST}/api/DeviceService/Devices({#Id})/InventoryDetails(\'serverArrayDisks\')', '$.InventoryInfo..Id', 'length', ''),
        ('{#DeviceName}: Total Disk Size', 'ome.device[{#Id}.disk.totalsize]', '{$INVENTORY_UPDATE_INTERVAL}', '3', 'Dell OME - Storage', 
         '{$OME_HOST}/api/DeviceService/Devices({#Id})/InventoryDetails(\'serverArrayDisks\')', '$.InventoryInfo..Size', '', 'GB'),
        
        # RAID
        ('{#DeviceName}: RAID Controller Count', 'ome.device[{#Id}.raid.count]', '{$INVENTORY_UPDATE_INTERVAL}', '3', 'Dell OME - RAID', 
         '{$OME_HOST}/api/DeviceService/Devices({#Id})/InventoryDetails(\'serverRaidControllers\')', '$.InventoryInfo..Id', 'length', ''),
        ('{#DeviceName}: RAID Controller Model', 'ome.device[{#Id}.raid.model]', '{$INVENTORY_UPDATE_INTERVAL}', '4', 'Dell OME - RAID', 
         '{$OME_HOST}/api/DeviceService/Devices({#Id})/InventoryDetails(\'serverRaidControllers\')', '$.InventoryInfo[0].Name', '', ''),
        
        # PSU
        ('{#DeviceName}: PSU Count', 'ome.device[{#Id}.psu.count]', '{$INVENTORY_UPDATE_INTERVAL}', '3', 'Dell OME - Power Supplies', 
         '{$OME_HOST}/api/DeviceService/Devices({#Id})/InventoryDetails(\'serverPowerSupplies\')', '$.InventoryInfo..Id', 'length', ''),
        ('{#DeviceName}: PSU Total Wattage', 'ome.device[{#Id}.psu.wattage]', '{$INVENTORY_UPDATE_INTERVAL}', '3', 'Dell OME - Power Supplies', 
         '{$OME_HOST}/api/DeviceService/Devices({#Id})/InventoryDetails(\'serverPowerSupplies\')', '$.InventoryInfo..Wattage', 'sum', 'W'),
        
        # Network
        ('{#DeviceName}: NIC Count', 'ome.device[{#Id}.nic.count]', '{$INVENTORY_UPDATE_INTERVAL}', '3', 'Dell OME - Network', 
         '{$OME_HOST}/api/DeviceService/Devices({#Id})/InventoryDetails(\'serverNetworkInterfaces\')', '$.InventoryInfo..Id', 'length', ''),
        
        # System - FIXED: units in <units> element
        ('{#DeviceName}: System Up Time', 'ome.device[{#Id}.uptime]', '5m', '3', 'Dell OME - System', 
         '{$OME_HOST}/api/DeviceService/Devices({#Id})', '$.SystemUpTime', '', 's'),
    ]
    ...
    for proto in prototypes:
        ...
        # Preprocessing - only JSONPATH and aggregation functions
        if proto[6]:  # JSONPATH
            ...
            if proto[7]:  # Aggregation function (length, sum)
                step2 = ET.SubElement(preprocessing, 'step')
                ET.SubElement(step2, 'type').text = proto[7]
        
        # Units in separate element
        if proto[8]:
...
if __name__ == '__main__':
    template = create_fixed_template()
    xml_string = prettify_xml(template)
    lines = [line for line in xml_string.split('\n') if line.strip()]
    xml_string = '\n'.join(lines)
    
    with open('template_dell_ome_2.0_fixed.xml', 'w', encoding='utf-8') as f:
        f.write(xml_string)
    
    print("Template v2.0 FIXED generated successfully!")
    print("File: template_dell_ome_2.0_fixed.xml")
